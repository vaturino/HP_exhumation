...
for j in range(ynum + 1): 
	...
	for i in range(xnum + 1):
		...
		if x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick-sed) and y <= (ymax -sed):
			C[ind,2]=1
		elif x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick-lcrust - sed) and y <= (ymax -cthick - sed):
			C[ind,3]=1
		elif x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick - lcrust) and y <= (ymax -cthick):
			C[ind,5]=1
		elif x >= (x_gap) and x <= (x_gap + x_SP - radius_outer) and y >= (ymax -cthick - lcrust - serp) and y <= (ymax -cthick - lcrust):
			C[ind,6]=1
		elif x >= (x_gap + x_SP) and x <= (xmax - x_gap) and y >= (ymax -cthick - lcrust):
			C[ind,5]= 1
		...
		if x > (x_gap + x_SP - radius_outer) and x < (x_gap + x_SP):
			...
			if ((x-x1)**2 + (y-y1)**2) < (radius_outer - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed)**2 and y > (ymax - cutoff): 
				...
				if angle > np.radians(90. - slab_dip):
					...
					C[ind,2]=1
			if ((x-x1)**2 + (y-y1)**2) < (radius_outer - cthick - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - sed - lcrust)**2 and y > (ymax - cutoff): 
				C[ind,3]= 0
				angle=np.arctan((y-y1)/(x-x1));
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
					C[ind,3]=1
			elif ((x-x1)**2 + (y-y1)**2) < (radius_outer - cthick - lcrust - sed)**2 and ((x-x1)**2 + (y-y1)**2) >= (radius_outer - cthick - lcrust - sed - serp)**2 and y > (ymax - cutoff): 
				C[ind,6]= 0
				angle=np.arctan((y-y1)/(x-x1));
				if angle > np.radians(90. - slab_dip):
					ynotch = radius_outer - np.sqrt((x-x1)**2 + (y-y1)**2)
					C[ind,6]=1
			elif ((x-x1)**2 + (y-y1)**2) >= radius_outer**2 and  y >= (ymax -cthick - lcrust):
				C[ind,5]=1
...
f.write("# Columns: x y compuc complc compop compopc compserp\n")
