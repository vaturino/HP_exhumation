...
output_loc = '/home/vturino/PhD/projects/exhumation/gz_outputs/'
solution = ''.join([models_loc, str(mod_name), '/particles.pvd'])
print(solution)

particles_loc = f'{output_loc}{mod_name}/particles/'
...
if not os.path.exists(particles_loc):
    os.mkdir(particles_loc)
...
for i in range(0, max_time, step):
    ...
    time = times[i]
    ofile = 'full.%d.gzip' % i
    ofiled = ''.join([particles_loc, ofile])
